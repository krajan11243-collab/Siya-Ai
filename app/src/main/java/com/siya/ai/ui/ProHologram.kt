...
fun ProHologram(active: Boolean, modifier: Modifier = Modifier) {
    ...
    Canvas(modifier.fillMaxSize()) {
        ...
        for (i in 0..8) { val y = cy - orbR * .72f + i * orbR * .18f; drawLine(HoloBlue.copy(alpha = .10f), Offset(cx - orbR * .95f, y), Offset(cx + orbR * .95f, y), strokeWidth = 1f) }
        for (i in -5..5) { val x = cx + i * orbR * .16f; drawLine(HoloCyan.copy(alpha = .09f), Offset(x, cy - orbR * 1.1f), Offset(x, cy + orbR * 1.1f), strokeWidth = 1f) }
        ...
        drawOval(brush = Brush.sweepGradient(listOf(HoloPurple, HoloCyan, HoloBlue, HoloPurple)), topLeft = Offset(cx - orbR * 1.16f, cy - orbR * .28f), size = Size(orbR * 2.32f, orbR * .56f), style = Stroke(width = 2f))
        drawOval(color = HoloCyan.copy(.40f), topLeft = Offset(cx - orbR * 1.03f, cy - orbR * .67f), size = Size(orbR * 2.06f, orbR * 1.34f), style = Stroke(width = 1.2f))
        drawArc(color = HoloCyan, startAngle = rotation, sweepAngle = 85f, useCenter = false, topLeft = Offset(cx - orbR * 1.12f, cy - orbR * 1.12f), size = Size(orbR * 2.24f, orbR * 2.24f), style = Stroke(width = 4f, cap = StrokeCap.Round))
        drawArc(color = HoloPurple, startAngle = rotation + 180f, sweepAngle = 65f, useCenter = false, topLeft = Offset(cx - orbR * 1.12f, cy - orbR * 1.12f), size = Size(orbR * 2.24f, orbR * 2.24f), style = Stroke(width = 4f, cap = StrokeCap.Round))
        ...
        drawOval(brush = Brush.sweepGradient(listOf(HoloBlue, HoloPurple, HoloCyan, HoloBlue)), topLeft = Offset(cx - orbR * 1.55f, platformY - orbR * .14f), size = Size(orbR * 3.10f, orbR * .28f), style = Stroke(width = 3f))
        drawOval(color = HoloCyan.copy(.55f), topLeft = Offset(cx - orbR * 1.30f, platformY - orbR * .07f), size = Size(orbR * 2.60f, orbR * .14f), style = Stroke(width = 2f))
